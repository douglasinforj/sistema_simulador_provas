...
class Alternativa(models.Model):
    questao = models.ForeignKey(Questao, on_delete=models.CASCADE, related_name='alternativa')
...
class Prova(models.Model):
    ...
    def calcular_nota(self):
        ...
        for questao in self.questoes.all():
            respostas_corretas = questao.alternativa.filter(is_correct=True)
            resposta_marcadas = Resposta.objects.filter(prova=self, alternativa__questao=questao)
            if resposta_marcadas.count() == respostas_corretas.count():
                todas_corretas = all(r.alternativa.is_correct for r in resposta_marcadas)
                if todas_corretas and resposta_marcadas.exists():
                    acerto += 1
